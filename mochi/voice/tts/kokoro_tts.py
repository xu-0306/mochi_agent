"""Kokoro TTS 包裝（bounded Phase 4，輸出 mono PCM16）。"""
...
KokoroSynthesizeCallable = Callable[[str, str, float, str], Any]


class KokoroTTS(BaseTTS):
    """Kokoro（KPipeline）最小封裝。"""
    ...
    def __init__(
        ...
        *,
        ...
        synthesize_callable: KokoroSynthesizeCallable | None = None,
    ) -> None:
        self.voice = voice
        ...
        if (
            ...
            and self._pipeline_factory is None
    ...
    async def synthesize(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(f"kokoro-tts synthesize unavailable [synthesize_failed]: {exc}") from exc
    ...
    def _synthesize_blocking(self, text: str, voice: str, speed: float) -> bytes:
        if self._synthesize_callable is not None:
            raw_audio = self._synthesize_callable(text, voice, speed, self.split_pattern)
            return self._normalize_to_pcm16_mono(raw_audio)

        runtime = self._ensure_runtime()
        if not callable(runtime):
            raise RuntimeError("kokoro-tts synthesize unavailable [factory_missing]")

        stream_or_audio = _call_with_supported_kwargs(
            runtime,
            text,
            voice=voice,
            speed=speed,
        ...
        return self._collect_and_normalize(stream_or_audio)
    ...
    def _ensure_runtime(self) -> Any:
        ...
        if self._dependency_error is not None:
            raise RuntimeError(
                "kokoro-tts synthesize unavailable [dependency_missing]: "
                f"{self._dependency_error}"
            ) from self._dependency_error
        if self._runtime_init_error is not None:
            raise RuntimeError(
                "kokoro-tts synthesize unavailable [runtime_init_failed]: "
        ...
        try:
            self._runtime = _call_with_supported_kwargs(self._pipeline_factory, lang_code=self.lang_code)
        except Exception as exc:
            self._runtime_init_error = exc
            raise RuntimeError(
                f"kokoro-tts synthesize unavailable [runtime_init_failed]: {exc}"
            ) from exc
        return self._runtime

    def _collect_and_normalize(self, stream_or_audio: Any) -> bytes:
        if isinstance(stream_or_audio, (bytes, bytearray)):
            return self._normalize_to_pcm16_mono(stream_or_audio)

        if isinstance(stream_or_audio, Iterable) and not isinstance(
            stream_or_audio,
            (str, bytes, bytearray, dict),
        ):
            chunks = bytearray()
            saw_chunk = False
            for item in stream_or_audio:
                saw_chunk = True
                chunks.extend(self._normalize_to_pcm16_mono(self._extract_audio_from_chunk(item)))
            if saw_chunk:
                return bytes(chunks)

        return self._normalize_to_pcm16_mono(stream_or_audio)

    def _extract_audio_from_chunk(self, item: Any) -> Any:
        ...
        if isinstance(item, tuple) and item:
            # Kokoro README 範例 tuple: (graphemes, phonemes, audio)
            return item[-1]
        return item
    ...
    def _normalize_bytes_audio(self, audio: bytes | bytearray) -> bytes:
        ...
        if _looks_like_wav(data):
            return _wav_to_pcm16_mono("kokoro-tts", data)
        if len(data) % 2 != 0:
            raise RuntimeError(
                "kokoro-tts synthesize unavailable [invalid_audio_format]: "
    ...
    def _normalize_numeric_audio(self, audio: Any) -> bytes:
        ...
        if isinstance(normalized, Sequence) and not isinstance(normalized, (str, bytes, bytearray)):
            ...
            if isinstance(first, Sequence) and not isinstance(first, (str, bytes, bytearray)):
                return _nested_numeric_to_pcm16_mono("kokoro-tts", normalized)
            if _is_numeric(first):
                return b"".join(pack("<h", _to_pcm16_sample(value)) for value in normalized)
        raise RuntimeError("kokoro-tts synthesize unavailable [invalid_audio_format]: unsupported audio type.")
