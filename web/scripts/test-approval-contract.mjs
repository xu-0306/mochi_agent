...
assert.match(api, /rule_persistence_status/)
assert.match(api, /'pending' \| 'retrying' \| 'delivered' \| 'failed'/)
assert.match(taskPanel, /Execution status:/)
assert.match(taskPanel, /Rule persistence:/)
assert.match(taskPanel, /do not resend the execution/)
