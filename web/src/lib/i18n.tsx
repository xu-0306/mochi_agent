...
const messages: Record<UILanguage, Record<TranslationKey, string>> = {
  'zh-TW': {
    ...
    'settings.modelConnection.ggufRuntimeMissing': 'GGUF 推理 runtime 尚未安裝。請先安裝 `llama-cpp-python`（例如執行 `uv sync --extra gguf`），再載入 GGUF 模型。',
    ...
    'settings.security.description': '調整執行自主度，以及 shell / 檔案工具的預設審批策略。',
    ...
    'settings.security.requireShellApproval': 'Shell 執行需審批',
  ...
  en: {
    ...
    'settings.modelConnection.ggufRuntimeMissing': 'The GGUF inference runtime is not installed. Install `llama-cpp-python` first, for example with `uv sync --extra gguf`, before loading a GGUF model.',
    ...
    'settings.security.description': 'Tune autonomy level and the default approval policy for shell and file tools.',
    ...
    'settings.security.requireShellApproval': 'Require approval for shell execution',
