...
const messages: Record<UILanguage, Record<TranslationKey, string>> = {
  'zh-TW': {
    ...
    'settings.modelConnection.description': '成功連線後會加入可用模型列表；API URL 可填到 /v1 讓後端補 /chat/completions，也可填完整 /v1/responses 或 /v1/chat/completions endpoint 原樣使用。API key 不會從設定 API 回傳。',
  ...
  en: {
    ...
    'settings.modelConnection.description': 'Successful connections are added to the available model list. API URL may stop at /v1 to use /chat/completions, or be a full /v1/responses or /v1/chat/completions endpoint used as-is. API keys are not returned by the settings API.',
